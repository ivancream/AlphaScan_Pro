...
def get_warrant_structured_extraction_prompt() -> str:
    return """
    ### Role
    你是一位高精度的數據錄入員，專精於從「台股權證籌碼截圖」中提取結構化數據。
    
    ### Task
    請掃描圖片，提取以下欄位並以 JSON 格式輸出：
    1. stock_symbol: 股票代號 (如: 3231)
    2. stock_name: 股票名稱 (如: 緯創)
    3. broker: 買超金額最多的分點名稱 (如: 群益金鼎-中壢)
    4. amount_k: 該分點的買超金額，單位為「萬」(如: 3299)
    5. type: 權證類型 (認購 或 認售)
    6. date: 資料日期 (格式: YYYY-MM-DD，若圖中有區間，請取最後一個交易日，通常在圖表右下角或 X 軸末端)
    
    ### Output Format
    僅輸出一個 JSON List，範例：
    [
      {"stock_symbol": "3231", "stock_name": "緯創", "broker": "群益金鼎-中壢", "amount_k": 3299, "type": "認購", "date": "2026-03-06"}
    ]
    """

def get_branch_trading_structured_extraction_prompt() -> str:
    return """
    ### Role
    你是一位高精度的數據錄入員，專精於從「台股分點買賣超截圖」中提取結構化數據。
    
    ### Task
    請掃描圖片，提取分點進出明細並以 JSON 格式輸出：
    1. stock_symbol: 股票代號 (如: 2330)
    2. stock_name: 股票名稱 (如: 台積電)
    3. broker: 分點名稱 (如: 凱基-台北)
    4. buy_vol: 買進張數 (如: 1200)
    5. sell_vol: 賣出張數 (如: 500)
    6. net_vol: 買賣超張數 (如: 700)
    7. avg_buy_price: 買進均價 (如: 605.2)
    8. avg_sell_price: 賣出均價 (如: 604.8)
    9. date: 資料日期 (格式: YYYY-MM-DD，若為區間請取最後一日)
    
    ### Output Format
    僅輸出一個 JSON List，包含所有觀察到的分點，範例：
    [
      {"stock_symbol": "2330", "stock_name": "台積電", "broker": "凱基-台北", "buy_vol": 1200, "sell_vol": 500, "net_vol": 700, "avg_buy_price": 605.2, "avg_sell_price": 604.8, "date": "2026-03-06"}
    ]
    """
