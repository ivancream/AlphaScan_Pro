...
set "SCRIPT_DIR=%~dp0"
REM Resolve project root from scripts/dev/start-dev.bat -> ../../
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%kill-dev-ports.ps1" -ProjectRoot "%ROOT%"
ping -n 2 127.0.0.1 >nul

echo Starting backend :8000 and frontend :1420 ...
start "AlphaScan Backend" cmd /k pushd "%ROOT%" ^&^& call .venv\Scripts\activate.bat ^&^& uvicorn backend.main:app --port 8000
